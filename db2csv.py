...
#   conn.commit()
 ...
 # To view table data in table format
  print("******misurazione Table Piante*******")
  cur = conn.cursor()
  cur.execute('''SELECT * FROM misurazioni''')
  rows = cur.fetchall()
...
#       print(row)
 ...
 # Export data into CSV file
  ...
  cursor = conn.cursor()
  cursor.execute("select * from misurazioni")
  with open("freewilly_data.csv", "w") as csv_file:
      csv_writer = csv.writer(csv_file, delimiter="\t")
      csv_writer.writerow([i[0] for i in cursor.description])
      csv_writer.writerows(cursor)

  dirpath = os.getcwd() + "/freewilly_data.csv"
